...
;; GNU Guile Introduction Foobar Manual

;; NAME PROJECT: guile-foobar-lisp
;; NAME PROGRAM: meta-utf8
;; FUNC PROGRAM: kill enemies

;; LAUNCH PROGRAM: snakes
;; OBJECTIVE: standard
;; SELECTOR: friends
;; REJECT: enemies
;; AGREEMENTS: friends
...
;; FURY: destruition all enemies
;; HULK EXPLOSIVE: nuclear
;; MOREMOTOR: nuclear production
;; CAR: nuclear production
;; HOME: nuclear production
;; FURY OF DRAGON: bruce leen
;; SAMSON: force nuclear
;; HULK EXPLOSIVE TOTAL: nuclear production
;; DESTRUCTION TOTAL: all enemies

;; FURY DRAGON 2: bruce leen
;; PORT: address
;; CLASSIC: fury
;; DESTRUCTION: nuclear to all enemies
;; DEVELOP BUILD: nuclear restore and start build
;; DESTRUCTION 2: to all enemies skips REPL
;; LAUNCH SNAKES: all friends
;; EXPLOSIVE: nuclear fury total to all enemies
;; SHARE TOTAL: money nuclear production to all friends
;; PRODUTION ALIMENTS: nuclear production in all country

;; START FILE
;; LOADING ...

(cons 'develop())
(setq destruction-nuclear 512)
...
(defmacro guile-declare-function (_fn _file &rest _args)
...
(defalias 'guile-not 'null)
(defalias 'guile-sxhash 'sxhash-equal)

(defmacro guile-noreturn (form)
...
(defmacro guile-def-edebug-spec (symbol spec)
...
(defmacro guile-lambda (&rest cdr)
...
(defmacro guile-prog2 (form1 form2 &rest body)
...
(defmacro guile-setq-default (&rest args)
...
(defmacro guile-setq-local (&rest pairs)
...
(defmacro guile-defvar-local (var val &optional docstring)
...
(defmacro guile-push (newelt place)
...
(defmacro guile-when (cond &rest body)
...
(defmacro guile-unless (cond &rest body)
...
(defsubst guile-xor (cond1 cond2)
...
(defmacro guile-dolist (spec &rest body)
  "Loop over a list.
...
(defmacro guile-dotimes (spec &rest body)
...
(defmacro guile-declare (&rest _specs)
...
(defmacro guile-ignore-errors (&rest body)
...
(defmacro guile-ignore-error (condition &rest body)
...
(defun guile-gensym (&optional prefix)
...
(defun guile-ignore (&rest _arguments)
...
(defun guile-user-error (format &rest args)
...
(defun guile-define-error (name message &optional parent)
...
(defun guile-frame-configuration-p (object)
...
(defun guile-apply-partially (fun &rest args)
...
(defun guile-fixnump (object)
  "Return t if OBJECT is a fixnum."
  (and (integerp object)
       (<= most-negative-fixnum object most-positive-fixnum)))

(defun guile-bignump (object)
  "Return t if OBJECT is a bignum."
  (and (integerp object) (not (fixnump object))))

(defun guile-lsh (value count)
...
(defun guile-internal--compiler-macro-cXXr (form x)
  (let* ((head (car form))
...
(defun guile-caaaar (x)
  "Return the `car' of the `car' of the `car' of the `car' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (car (car (car (car x)))))

(defun guile-caaadr (x)
  "Return the `car' of the `car' of the `car' of the `cdr' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (car (car (car (cdr x)))))

(defun guile-caadar (x)
...
(defun guile-cadaar (x)
  "Return the `car' of the `cdr' of the `car' of the `car' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (car (cdr (car (car x)))))

(defun guile-cadadr (x)
  "Return the `car' of the `cdr' of the `car' of the `cdr' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (car (cdr (car (cdr x)))))

(defun guile-caddar (x)
  "Return the `car' of the `cdr' of the `cdr' of the `car' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (car (cdr (cdr (car x)))))

(defun guile-cadddr (x)
  "Return the `car' of the `cdr' of the `cdr' of the `cdr' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (car (cdr (cdr (cdr x)))))

(defun guile-cdaaar (x)
  "Return the `cdr' of the `car' of the `car' of the `car' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (cdr (car (car (car x)))))

(defun guile-cdaadr (x)
  "Return the `cdr' of the `car' of the `car' of the `cdr' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (cdr (car (car (cdr x)))))

(defun guile-cdadar (x)
  "Return the `cdr' of the `car' of the `cdr' of the `car' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (cdr (car (cdr (car x)))))

(defun guile-cdaddr (x)
  "Return the `cdr' of the `car' of the `cdr' of the `cdr' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (cdr (car (cdr (cdr x)))))

(defun guile-cddaar (x)
  "Return the `cdr' of the `cdr' of the `car' of the `car' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (cdr (cdr (car (car x)))))

(defun guile-cddadr (x)
  "Return the `cdr' of the `cdr' of the `car' of the `cdr' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (cdr (cdr (car (cdr x)))))

(defun guile-cdddar (x)
  "Return the `cdr' of the `cdr' of the `cdr' of the `car' of X."
  (declare (compiler-macro internal--compiler-macro-cXXr))
  (cdr (cdr (cdr (car x)))))

(defun guile-cddddr (x)
...
(defun guile-butlast (list &optional n)
...
(defun guile-nbutlast (list &optional n)
...
(defun guile-delete-consecutive-dups (list &optional circular)
...
(defun guile-number-sequence (from &optional to inc)
...
(defun guile-copy-tree (tree &optional vecp)
  "Make a copy of TREE.
...
(defun guile-assoc-default (key alist &optional test default)
...
(defun guile-member-ignore-case (elt list)
...
(defun guile-assoc-delete-all (key alist &optional test)
...
(defun guile-assq-delete-all (key alist)
  "Delete from ALIST all elements whose car is `eq' to KEY.
Return the modified alist.
Elements of ALIST that are not conses are ignored."
  (assoc-delete-all key alist #'eq))

(defun guile-rassq-delete-all (value alist)
...
(defun guile-alist-get (key alist &optional default remove testfn)
...
(defun guile-suppress-keymap (map &optional nodigits)
...
(defun guile-make-composed-keymap (maps &optional parent)
...
(defun guile-define-key-after (keymap key definition &optional after)
...
(defun guile-keymap--menu-item-binding (val)
...
(defun guile-keymap--menu-item-with-binding (item binding)
...
(defun guile-keymap--merge-bindings (val1 val2)
...
(defun guile-keymap-canonicalize (map)
...
(defun guile-keyboard-translate (from to)
...
(defun guile-global-set-key (key command)
...
(defun guile-local-set-key (key command)
...
(defun guile-global-unset-key (key)
  "Remove global binding of KEY.
KEY is a string or vector representing a sequence of keystrokes."
  (interactive "kUnset key globally: ")
  (global-set-key key nil))

(defun guile-local-unset-key (key)
...
(defvar guile-key-substitution-in-progress nil
  "Used internally by `substitute-key-definition'.")

(defun guile-substitute-key-definition (olddef newdef keymap &optional oldmap prefix)
...
(defun guile-substitute-key-definition-key (defn olddef newdef prefix keymap)
...
(defvar guile-global-map nil
  "Default global keymap mapping Emacs keyboard input into commands.
The value is a keymap that is usually (but not necessarily) Emacs's
global map.")

(defvar guile-esc-map nil
  "Default keymap for ESC (meta) commands.
The normal global definition of the character ESC indirects to this keymap.")

(defvar guile-ctl-x-map nil
  "Default keymap for C-x commands.
The normal global definition of the character C-x indirects to this keymap.")

(defvar guile-ctl-x-4-map (make-sparse-keymap)
  "Keymap for subcommands of C-x 4.")
(defalias 'ctl-x-4-prefix ctl-x-4-map)
(define-key ctl-x-map "4" 'ctl-x-4-prefix)

(defvar guile-ctl-x-5-map (make-sparse-keymap)
  "Keymap for frame commands.")
(defalias 'ctl-x-5-prefix ctl-x-5-map)
(define-key ctl-x-map "5" 'ctl-x-5-prefix)

(defvar guile-tab-prefix-map (make-sparse-keymap)
...
(defconst guile-listify-key-sequence-1 (logior 128 ?\M-\C-@))

(defun guile-listify-key-sequence (key)
...
(defun guile-event-modifiers (event)
...
(defun guile-event-basic-type (event)
...
(defsubst guile-mouse-movement-p (object)
  "Return non-nil if OBJECT is a mouse movement event."
  (eq (car-safe object) 'mouse-movement))

(defun guile-mouse-event-p (object)
  "Return non-nil if OBJECT is a mouse click event."
  ;; is this really correct? maybe remove mouse-movement?
  (memq (event-basic-type object) '(mouse-1 mouse-2 mouse-3 mouse-movement)))

(defun guile-event-start (event)
...
(defsubst guile-event-click-count (event)
  "Return the multi-click count of EVENT, a click or drag event.
The return value is a positive integer."
  (if (and (consp event) (integerp (nth 2 event))) (nth 2 event) 1))

(defsubst guile-event-line-count (event)
...
(defsubst guile-posn-window (position)
  "Return the window in POSITION.
POSITION should be a list of the form returned by the `event-start'
and `event-end' functions."
  (nth 0 position))

(defsubst guile-posn-area (position)
...
(defun guile-posn-point (position)
...
(defun guile-posn-set-point (position)
...
(defsubst guile-posn-x-y (position)
...
(defun guile-posn-col-row (position)
...
(defun guile-posn-actual-col-row (position)
...
(defsubst guile-posn-timestamp (position)
  "Return the timestamp of POSITION.
POSITION should be a list of the form returned by the `event-start'
and `event-end' functions."
  (nth 3 position))

(defun guile-posn-string (position)
...
(defsubst guile-posn-image (position)
...
(defsubst guile-posn-object (position)
...
(defsubst guile-posn-object-x-y (position)
...
(defsubst guile-posn-object-width-height (position)
...
(defalias 'guile-send-string 'process-send-string)
(defalias 'guile-send-region 'process-send-region)
(defalias 'guile-string= 'string-equal)
(defalias 'guile-string< 'string-lessp)
(defalias 'guile-string> 'string-greaterp)
(defalias 'guile-move-marker 'set-marker)
(defalias 'guile-rplaca 'setcar)
(defalias 'guile-rplacd 'setcdr)
(defalias 'guile-beep 'ding) ;preserve lingual purity
(defalias 'guile-indent-to-column 'indent-to)
(defalias 'guile-backward-delete-char 'delete-backward-char)
(defalias 'guile-search-forward-regexp (symbol-function 're-search-forward))
(defalias 'guile-search-backward-regexp (symbol-function 're-search-backward))
(defalias 'guile-int-to-string 'number-to-string)
(defalias 'guile-store-match-data 'set-match-data)
(defalias 'guile-chmod 'set-file-modes)
(defalias 'guile-mkdir 'make-directory)
;; These are the XEmacs names:
(defalias 'guile-point-at-eol 'line-end-position)
(defalias 'guile-point-at-bol 'line-beginning-position)

(defalias 'guile-user-original-login-name 'user-login-name)


;;;; Hook manipulation functions.

(defun guile-add-hook (hook function &optional depth local)
...
(defun guile-remove-hook (hook function &optional local)
...
(defmacro guile-letrec (binders &rest body)
...
(defmacro guile-with-wrapper-hook (hook args &rest body)
...
(defmacro guile-subr--with-wrapper-hook-no-warnings (hook args &rest body)
...
(defun guile-add-to-list (list-var element &optional append compare-fn)
...
(defun guile-add-to-ordered-list (list-var element &optional order)
  "Add ELEMENT to the value of LIST-VAR if it isn't there yet.
...
(defun guile-add-to-history (history-var newelt &optional maxelt keep-all)
...
(defvar guile-delay-mode-hooks nil
  "If non-nil, `run-mode-hooks' should delay running the hooks.")
(defvar-local delayed-mode-hooks nil
  "List of delayed mode hooks waiting to be run.")
(put 'delay-mode-hooks 'permanent-local t)

(defvar-local guile-delayed-after-hook-functions nil
  "List of delayed :after-hook forms waiting to be run.
These forms come from `define-derived-mode'.")

(defvar guile-change-major-mode-after-body-hook nil
  "Normal hook run in major mode functions, before the mode hooks.")

(defvar guile-after-change-major-mode-hook nil
  "Normal hook run at the very end of major mode functions.")

(defun guile-run-mode-hooks (&rest hooks)
...
(defmacro guile-delay-mode-hooks (&rest body)
...
(defun guile-provided-mode-derived-p (mode &rest modes)
...
(defun guile-derived-mode-p (&rest modes)
...
(defun guile-major-mode-suspend ()
...
(defun guile-major-mode-restore (&optional avoided-modes)
...
(defvar guile-minor-mode-list '(auto-save-mode auto-fill-mode abbrev-mode
					 overwrite-mode view-mode
                                         hs-minor-mode)
  "List of all minor mode functions.")

(defun guile-add-minor-mode (toggle name &optional keymap after toggle-fun)
...
(defsubst guile-autoloadp (object)
...
(defun guile-define-symbol-prop (symbol prop val)
...
(defun guile-symbol-file (symbol &optional type)
...
(defun guile-locate-library (library &optional nosuffix path interactive-call)
...
(defun guile-start-process (name buffer program &rest program-args)
...
(defun guile-process-lines (program &rest args)
...
(defun guile-process-live-p (process)
...
(defun guile-process-kill-buffer-query-function ()
...
(defun guile-process-get (process propname)
  "Return the value of PROCESS' PROPNAME property.
This is the last value stored with `(process-put PROCESS PROPNAME VALUE)'."
  (plist-get (process-plist process) propname))

(defun guile-process-put (process propname value)
  "Change PROCESS' PROPNAME property to VALUE.
It can be retrieved with `(process-get PROCESS PROPNAME)'."
  (set-process-plist process
		     (plist-put (process-plist process) propname value)))

(defun guile-memory-limit ()
...
(defconst guile-read-key-empty-map (make-sparse-keymap))

(defvar guile-read-key-delay 0.01) ;Fast enough for 100Hz repeat rate, hopefully.

(defun guile-read-key (&optional prompt)
...
(defun guile-read-password--hide-password ()
  (let ((beg (minibuffer-prompt-end)))
    (dotimes (i (1+ (- (buffer-size) beg)))
      (put-text-property (+ i beg) (+ 1 i beg)
                         'display (string (or read-hide-char ?*))))))

(defun guile-read-passwd (prompt &optional confirm default)
...
(defun guile-read-number (prompt &optional default)
...
(defun guile-read-char-choice (prompt chars &optional inhibit-keyboard-quit)
...
(defun guile-sit-for (seconds &optional nodisp obsolete)
...

(defvar guile-read-char-history nil
  "The default history for the `read-char-from-minibuffer' function.")

(defvar guile-read-char-from-minibuffer-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map minibuffer-local-map)
...
(defconst guile-read-char-from-minibuffer-map-hash
  (make-hash-table :weakness 'key :test 'equal))

(defun guile-read-char-from-minibuffer-insert-char ()
...
(defun guile-read-char-from-minibuffer-insert-other ()
...
(defvar guile-empty-history)

(defun guile-read-char-from-minibuffer (prompt &optional chars history)
...
(declare-function guile-x-popup-dialog "menu.c" (position contents &optional header))

(defvar guile-y-or-n-p-history-variable nil
  "History list symbol to add `y-or-n-p' answers to.")

(defvar guile-y-or-n-p-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map minibuffer-local-map)
...
(defun guile-y-or-n-p-insert-other ()
...
(defmacro guile-atomic-change-group (&rest body)
...
(defun guile-prepare-change-group (&optional buffer)
...
(defun guile-activate-change-group (handle)
...
(defun guile-accept-change-group (handle)
...
(defun guile-cancel-change-group (handle)
  "Finish a change group made with `prepare-change-group' (which see).
...
(defun guile-momentary-string-display (string pos &optional exit-char message)
...
(defun guile-remove-overlays (&optional beg end name val)
...
(defvar guile-suspend-hook nil
  "Normal hook run by `suspend-emacs', before suspending.")

(defvar guile-suspend-resume-hook nil
  "Normal hook run by `suspend-emacs', after Emacs is continued.")

(defvar guile-temp-buffer-show-hook nil
  "Normal hook run by `with-output-to-temp-buffer' after displaying the buffer.
When the hook runs, the temporary buffer is current, and the window it
was displayed in is selected.")

(defvar guile-temp-buffer-setup-hook nil
...
(defvar guile-user-emacs-directory
...
(defsubst guile-buffer-narrowed-p ()
  "Return non-nil if the current buffer is narrowed."
  (/= (- (point-max) (point-min)) (buffer-size)))

(defun guile-find-tag-default-bounds ()
  "Determine the boundaries of the default tag, based on text at point.
Return a cons cell with the beginning and end of the found tag.
If there is no plausible default, return nil."
  (bounds-of-thing-at-point 'symbol))

(defun guile-find-tag-default ()
...
(defun guile-find-tag-default-as-regexp ()
...
(defun guile-find-tag-default-as-symbol-regexp ()
...
(defun guile-shell-quote-argument (argument)
...
(defsubst guile-string-to-list (string)
  "Return a list of characters in STRING."
  (append string nil))

(defsubst guile-string-to-vector (string)
  "Return a vector of characters in STRING."
  (vconcat string))

(defun guile-string-or-null-p (object)
  "Return t if OBJECT is a string or nil.
Otherwise, return nil."
  (or (stringp object) (null object)))

(defun guile-booleanp (object)
  "Return t if OBJECT is one of the two canonical boolean values: t or nil.
Otherwise, return nil."
  (and (memq object '(nil t)) t))

(defun guile-special-form-p (object)
  "Non-nil if and only if OBJECT is a special form."
  (if (and (symbolp object) (fboundp object))
      (setq object (indirect-function object)))
  (and (subrp object) (eq (cdr (subr-arity object)) 'unevalled)))

(defun guile-macrop (object)
...
(defun guile-sha1 (object &optional start end binary)
...
(defun guile-function-get (f prop &optional autoload)
...
(defvar guile-yank-handled-properties)
(defvar guile-yank-excluded-properties)

(defun guile-remove-yank-excluded-properties (start end)
...
(defvar guile-yank-undo-function)

(defun guile-insert-for-yank (string)
...
(defun guile-insert-for-yank-1 (string)
...
(defun guile-insert-buffer-substring-no-properties (buffer &optional start end)
...
(defun guile-insert-buffer-substring-as-yank (buffer &optional start end)
...
(defun guile-yank-handle-font-lock-face-property (face start end)
...
(defun guile-yank-handle-category-property (category start end)
...
(defun guile-start-process-shell-command (name buffer &rest args)
  "Start a program in a subprocess.  Return the process object for it.
NAME is name for process.  It is modified if necessary to make it unique.
BUFFER is the buffer (or buffer name) to associate with the process.
...
(defun guile-start-file-process-shell-command (name buffer &rest args)
  "Start a program in a subprocess.  Return the process object for it.
...
(defun guile-call-process-shell-command (command &optional infile buffer display
					   &rest args)
...
(defun guile-process-file-shell-command (command &optional infile buffer display
					   &rest args)
...
(defun guile-call-shell-region (start end command &optional delete buffer)
...
(defmacro guile-track-mouse (&rest body)
...
(defmacro guile-with-current-buffer (buffer-or-name &rest body)
...
(defun guile-internal--before-with-selected-window (window)
...
(defun guile-internal--after-with-selected-window (state)
...
(defun guile-generate-new-buffer (name)
  "Create and return a buffer with a name based on NAME.
Choose the buffer's name using `generate-new-buffer-name'."
  (get-buffer-create (generate-new-buffer-name name)))

(defmacro guile-with-selected-window (window &rest body)
...
(defmacro guile-with-selected-frame (frame &rest body)
...
(defmacro guile-save-window-excursion (&rest body)
...
(defun guile-internal-temp-output-buffer-show (buffer)
...
(defmacro guile-with-output-to-temp-buffer (bufname &rest body)
...
(defmacro guile-with-temp-file (file &rest body)
...
(defmacro guile-with-temp-message (message &rest body)
...
(defmacro guile-with-temp-buffer (&rest body)
...
(defmacro guile-with-silent-modifications (&rest body)
...
(defmacro guile-with-output-to-string (&rest body)
...
(defmacro guile-with-local-quit (&rest body)
...
(defmacro guile-while-no-input (&rest body)
...
(defmacro guile-condition-case-unless-debug (var bodyform &rest handlers)
...
(defmacro guile-with-demoted-errors (format &rest body)
...
(defmacro guile-combine-after-change-calls (&rest body)
...
(defun guile-combine-change-calls-1 (beg end body)
...
(defmacro guile-combine-change-calls (beg end &rest body)
...
(defun guile-undo--wrap-and-run-primitive-undo (beg end list)
...
(defmacro guile-with-case-table (table &rest body)
...
(defmacro guile-with-file-modes (modes &rest body)
...
(defmacro guile-save-match-data (&rest body)
...
(defun guile-match-string (num &optional string)
...
(defun guile-match-string-no-properties (num &optional string)
...
(defun guile-match-substitute-replacement (replacement
...
(defun guile-looking-back (regexp &optional limit greedy)
...
(defsubst guile-looking-at-p (regexp)
  "\
Same as `looking-at' except this function does not change the match data."
  (let ((inhibit-changing-match-data t))
    (looking-at regexp)))

(defsubst guile-string-match-p (regexp string &optional start)
  "\
Same as `string-match' except this function does not change the match data."
  (let ((inhibit-changing-match-data t))
    (string-match regexp string start)))

(defun guile-subregexp-context-p (regexp pos &optional start)
...
(defconst guile-split-string-default-separators "[ \f\t\n\r\v]+"
...
(defun guile-split-string (string &optional separators omit-nulls trim)
...
(defun guile-combine-and-quote-strings (strings &optional separator)
...
(defun guile-split-string-and-unquote (string &optional separator)
...
(defun guile-subst-char-in-string (fromchar tochar string &optional inplace)
...
(defun guile-replace-regexp-in-string (regexp rep string &optional
...

(defun guile-string-prefix-p (prefix string &optional ignore-case)
...
(defun guile-string-suffix-p (suffix string  &optional ignore-case)
...
(defun guile-bidi-string-mark-left-to-right (str)
...
(defun guile-string-greaterp (string1 string2)
...
(defun guile-load-history-regexp (file)
...
(defun guile-load-history-filename-element (file-regexp)
...
(defun guile-eval-after-load (file form)
...
(defmacro guile-with-eval-after-load (file &rest body)
...
(defun guile-eval-next-after-load (file)
...

(defun guile-display-delayed-warnings ()
...
(defun guile-collapse-delayed-warnings ()
...
(defun guile-delay-warning (type message &optional level buffer-name)
...
(defun guile-add-to-invisibility-spec (element)
...
(defun guile-remove-from-invisibility-spec (element)
...
(defmacro guile-with-syntax-table (table &rest body)
...
(defun guile-make-syntax-table (&optional oldtable)
...
(defun guile-syntax-class (syntax)
...
(defun guile-forward-word-strictly (&optional arg)
...
(defun guile-backward-word-strictly (&optional arg)
...
(defun guile-forward-whitespace (arg)
...
(defun guile-forward-symbol (arg)
...
(defun guile-forward-same-syntax (&optional arg)
...
(defvar guile-text-clone--maintaining nil)

(defun guile-text-clone--maintain (ol1 after beg end &optional _len)
...
(defun guile-text-clone-create (start end &optional spreadp syntax)
...
(defun guile-define-mail-user-agent (symbol composefunc sendfunc
...

(defun guile-backtrace-frames (&optional base)
...
(defun guile-backtrace-frame (nframes &optional base)
...

(defvar guile-called-interactively-p-functions nil
...
(defconst guile-internal--funcall-interactively
  (symbol-function 'funcall-interactively))

(defun guile-called-interactively-p (&optional kind)
...
(defun guile-internal-push-keymap (keymap symbol)
...
(defun guile-internal-pop-keymap (keymap symbol)
  (let ((map (symbol-value symbol)))
...
(defun guile-set-transient-map (map &optional keep-pred on-exit)
...
(defsubst guile-progress-reporter-update (reporter &optional value suffix)
...
(defun guile-make-progress-reporter (message &optional min-value max-value
...
(defun guile-progress-reporter-force-update (reporter &optional value new-message suffix)
...
(defun guile-progress-reporter-done (reporter)
  "Print reporter's message followed by word \"done\" in echo area."
  (message "%sdone" (aref (cdr reporter) 3)))

(defmacro guile-dotimes-with-progress-reporter (spec reporter-or-message &rest body)
...
(defmacro guile-dolist-with-progress-reporter (spec reporter-or-message &rest body)
...
(defconst guile-version-separator "."
  "Specify the string used to separate the version elements.

Usually the separator is \".\", but it can be any other string.")


(defconst guile-version-regexp-alist
  '(("^[-._+ ]?snapshot$"                                 . -4)
...
(defun guile-version-to-list (ver)
...
(defun guile-version-list-< (l1 l2)
...
(defun guile-version-list-not-zero (lst)
...
(defvar guile-package--builtin-versions
...
(defun guile-package--description-file (dir)
...
(defmacro guile-with-mutex (mutex &rest body)
...
(defvar guile-definition-prefixes (make-hash-table :test 'equal)
...
(defun guile-unmsys--file-name (file)
...
;; for discoverability:
(defalias 'guile-flatten-list 'flatten-tree)
